...
public class Hand {
    ...
    private int getMaxNumber(ArrayList<String> number) {
        ...
        // Unique values of "number"
        Set<String> set = new HashSet<>(number);
        
        // count the occurrences of each number in the hand given
        for (String v: set) {
            // occurrences of the value "v" in "number"
            n = Collections.frequency(number, v);
            // integer value of the card
            aux_n = this.getNumberAsInteger(v);
            
            // ** CHECK COMBINATIONS FIRST --> full house or two pair                
            ...
            } else if (max_n == 2 && n == 2) {// && this.secondary_rank != aux_n) {
                // set the aux_rank
            ...
            // four of a kind, three of a kind or pair
            } else if (n == 4 || n == 3 || n == 2) {
                max_n = n;
                this.secondary_rank = aux_n;
            // high card
